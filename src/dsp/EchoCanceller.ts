export class SignalGenerator {
  private phase = 0;
  getNextSample(frequency = 250): number {
    this.phase += (Math.PI * 2 * frequency) / 8000;
    if (this.phase > Math.PI * 2) this.phase -= Math.PI * 2;
    const raw = Math.sin(this.phase) + 0.5 * Math.sin(this.phase * 2.3);
    const envelope = Math.max(0, Math.sin(this.phase * 0.1)) ** 4;
    return raw * envelope;
  }
}

export class DelayLine {
  private buffer: Float32Array;
  private ptr: number = 0;
  constructor(maxDelay: number) {
    this.buffer = new Float32Array(maxDelay);
  }
  process(input: number, delay: number, attenuation: number): number {
    const outIndex = (this.ptr - Math.floor(delay) + this.buffer.length) % this.buffer.length;
    const delayedSample = this.buffer[outIndex];
    this.buffer[this.ptr] = input;
    this.ptr = (this.ptr + 1) % this.buffer.length;
    return delayedSample * attenuation;
  }
}

export class MetricCalculator {
  private dPower = 0.001;
  private ePower = 0.001;
  private smoothing = 0.99;
  update(d: number, e: number): number {
    this.dPower = this.smoothing * this.dPower + (1 - this.smoothing) * (d * d);
    this.ePower = this.smoothing * this.ePower + (1 - this.smoothing) * (e * e);
    const erle = 10 * Math.log10(this.dPower / this.ePower);
    return Math.max(0, erle);
  }
}

export class AdaptiveFilter {
  public weights: Float32Array;
  private xBuffer: Float32Array;
  private mu: number;

  constructor(numTaps: number, learningRate: number = 0.01) {
    this.weights = new Float32Array(numTaps);
    this.xBuffer = new Float32Array(numTaps);
    this.mu = learningRate;
  }

  process(x: number, d: number): { estimatedEcho: number; error: number } {
    for (let i = this.xBuffer.length - 1; i > 0; i--) {
      this.xBuffer[i] = this.xBuffer[i - 1];
    }
    this.xBuffer[0] = x;

    let y_hat = 0;
    for (let i = 0; i < this.weights.length; i++) {
      y_hat += this.weights[i] * this.xBuffer[i];
    }

    const error = d - y_hat;

    let norm = 0.001;
    for (let i = 0; i < this.xBuffer.length; i++) {
      norm += this.xBuffer[i] * this.xBuffer[i];
    }
    const stepSize = this.mu / norm;

    for (let i = 0; i < this.weights.length; i++) {
      this.weights[i] += stepSize * error * this.xBuffer[i];
    }

    return { estimatedEcho: y_hat, error };
  }

  getEstimatedDelayAndAlpha(): { estDelay: number; estAlpha: number } {
    let maxIdx = 1;
    let maxVal = 0;
    for (let i = 1; i < this.weights.length; i++) {
      if (Math.abs(this.weights[i]) > Math.abs(maxVal)) {
        maxVal = this.weights[i];
        maxIdx = i;
      }
    }
    return { estDelay: maxIdx, estAlpha: maxVal };
  }
}
